#!/usr/bin/env python3
"""
🚀 ULTIMATE PROFIT MASTER MEGA-SYSTEM v18.1 (TITAN-ENHANCED EDITION)
...
🔄 Complete Refactored Version with Concurrency & Incremental Saves
"""
...
try:
    ...
    import pandas as pd
    import psutil
    import pytz                     # required for timezone logic in MegaContentEngine
    ...
    def _create_groq_client(self, key):
        class GroqClientWrapper:
            ...
            async def generate_content(self, prompt, max_tokens=4000):
                ...
                # Added robust timeout to prevent silent hanging
...
class MegaContentEngine:
    ...
    - 15-ቁልፍ ሮቴሽን (Round-Robin Relay Race) with Exponential Backoff
    ...
    def __init__(self, system):
        ...
        self.logger.info("🚀 MegaContentEngine Initialized with 15-Key Round-Robin System & Concurrency")
    ...
    async def _call_ai_with_round_robin(self, prompt, max_tokens=4000, phase_idx=0):
        ...
        # Allow up to 2 full rounds of retries
        for attempt in range(total_providers * 2):
            try:
                provider_idx = (phase_idx + attempt) % total_providers
                provider = self.ai_providers[provider_idx]

                self.logger.info(f"🔄 Round-Robin Attempt {attempt+1}/{total_providers*2}: "
                ...
                if hasattr(provider, 'generate_content'):
                    ...
                    self.logger.info(f"💤 Key {provider_idx+1} now resting...")
                    return result

            except Exception as e:
                error_msg = str(e).lower()
                if "429" in error_msg or "rate limit" in error_msg:
                    # Exponential Backoff logic implemented here
                    backoff = min(60, 2 ** attempt)
                    self.logger.warning(f"⚠️ Rate limit on Provider {provider_idx+1}, backing off {backoff}s...")
        ...
        self.logger.error("❌ All fallback keys failed after multiple rounds!")
        raise Exception(f"All fallback keys failed for Phase {phase_idx}")
    ...
    async def produce_single_country_sovereign_logic(self, topic: str, country: str) -> str:
        ...
        for idx, (phase_num, name, target_words) in enumerate(tasks):
            ...
            # Incremental State Saving Mechanism
            try:
                inc_dir = Path('outputs/incremental_saves')
                inc_dir.mkdir(parents=True, exist_ok=True)
                with open(inc_dir / f"{country}_phase_{phase_num}_backup.html", "w", encoding="utf-8") as f:
    ...
    async def produce_11_countries_mega_loop(self, topic: str):
        """ADDED: Produce mega content for all 11 high-value countries CONCURRENTLY."""
        target_countries = ['US', 'GB', 'DE', 'CA', 'AU', 'FR', 'JP', 'CH', 'NO', 'SE', 'ET']
        self.logger.info(f"🚀 Starting 11-Country Mega Loop CONCURRENTLY for topic: {topic}")
        
        # Concurrency Limiter: Process up to 4 countries at a time
        ...
        tasks = [process_country(country) for country in target_countries]
        await asyncio.gather(*tasks)
        self.logger.info("🎉 All 11 countries processed concurrently!")
    ...
    async def start_mega_loop(self, topic: str):
        ...
        self.logger.info(f"🚀 Starting Mega Loop for topic: {topic} (CONCURRENT MODE)")
        self.logger.info(f"🌍 Target countries: {', '.join(target_countries)}")

        semaphore = asyncio.Semaphore(4) # Run up to 4 countries simultaneously
        ...
        while True:
            ...
            async def process_hot_country(country):
                nonlocal total_predicted_revenue
                async with semaphore:
                    if self._is_hot_country_time(country):
                        ...
                        try:
                            ...
                            self.logger.info(f"⏸️  Cooling down for 5 minutes (CNN-style pacing) for {country}...")
            ...
            # Execute all countries concurrently based on the semaphore limit
            tasks = [process_hot_country(country) for country in target_countries]
            await asyncio.gather(*tasks)
            ...
            if active_productions:
                ...
                self.logger.info("😴 No countries in hot time - Sleeping for 1 minute (Energy Saving Mode)...")
