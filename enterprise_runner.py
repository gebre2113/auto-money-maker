...
# ========== 🔁 UNSTOPPABLE 15-KEY ROUND ROBIN – GROQ ONLY (Enhanced Backoff) ==========
class UnstoppableAIProvider:
    ...
    def __init__(self, config=None):
        ...
        self.failure_count = {}           # track consecutive failures per key
    ...
    async def generate_content(self, prompt: str, max_tokens: int = 4000) -> str:
        ...
        # Try each key up to two full rounds
        for attempt in range(len(self.groq_pool) * 2):
            ...
            if current_slot in self.key_blacklist:
                if now < self.key_blacklist[current_slot]:
                    continue  # still blacklisted
                else:
                    del self.key_blacklist[current_slot]
                    self.failure_count[current_slot] = 0  # reset failure count
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=160.0) as client:
                    ...
                    if resp.status_code == 200:
                        ...
                        self.failure_count[current_slot] = 0
                        return resp.json()['choices'][0]['message']['content']
                    elif resp.status_code == 429:
                        # Rate limit – increase blacklist duration exponentially
                        fail_cnt = self.failure_count.get(current_slot, 0) + 1
                        self.failure_count[current_slot] = fail_cnt
                        block_time = min(600, 180 * (2 ** (fail_cnt - 1)))  # 180, 360, 600 max
                        self.key_blacklist[current_slot] = now + block_time
                        self.logger.warning(f"🚫 Slot-{current_slot+1} Rate Limited. Blocked {block_time}s. Fail count: {fail_cnt}")
                        await asyncio.sleep(2)
                        continue
                    else:
                        # Other error – blacklist for 300s
...
class EnterpriseProductionOrchestrator:
    ...
    def _initialize_all_components(self):
        """🏢 ULTIMATE INTERACTIVE BRIDGE - Audio + Audio Ads + High Quality"""
        self.logger.info("🏢 Initializing Interactive Sovereign Bridge v6.0...")
        try:
            # 1. YouTube Hunter
            ...
            if profit_sys_class:
                ...
                # --- 🌉 THE INTERACTIVE SOVEREIGN BRIDGE v6.0 ---
                async def interactive_bridge(topic, country, omega_key_number=0):
                    self.logger.info(f"🎙️ Starting Interactive Production for {country}")
                    ...
                    # መ. አውዲዮ እና ማስታወቂያ ስክሪፕት (The Magic Script)
                    ...
                    def replace_table(match):
                        ...
                        ad_message = f"Based on this data, we recommend using {product_name}. Check the link in the article for a special 2026 offer."
                        if country == "ET":
                            ad_message = f"በዚህ ሰንጠረዥ መሠረት {product_name}ን እንዲጠቀሙ እንመክራለን። አሁኑኑ ሊንኩን በመጫን የቅናሽ ተጠቃሚ ይሁኑ።"
                        
                        button_html = f'''
                        <button onclick="playSectionWithAd('{table_id}', '{ad_message}')" 
                                style="background:#c5a059; color:white; padding:10px 15px; border-radius:5px; cursor:pointer; margin-bottom:10px; border:none; font-weight:bold; display:inline-block;">
                            🔊 Listen to Table & Recommendation
                        </button>
                        '''
                        table_html = match.group(0)
                        # Wrap table with id
                        wrapped_table = f'<div id="{table_id}">{table_html}</div>'
                    ...
                    # Add the audio script at the top
                    interactive_js = f"""
                    <script>
                    function playSectionWithAd(elementId, adText) {{
                    ...
                    return {
                        ...
                        'metrics': {'quality_score': 92.0, 'human_score': 95.0, 'enterprise_grade': True}
                    }

                # የጎደለውን ዘዴ ራነሩ ላይ በሜሞሪ መፍጠር
                self.content_system._process_country_enterprise = interactive_bridge
                self.logger.info("✅ Interactive Sovereign Bridge Logic Injected Successfully")
