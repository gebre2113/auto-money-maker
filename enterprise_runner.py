...
class SocialMediaManager:
    """
    🚀 ENTERPRISE MULTI-PLATFORM PUBLISHER v4.0 (MASTER STABLE)
    ምንም አይነት የስም ስህተት እንዳይፈጠር ሁሉንም ዘዴዎች አዋህዶ የያዘ
    """
    
    def __init__(self):
        self.logger = logging.getLogger("UltimateSocialManager")
        
        # 🔑 መረጃዎችን ከ GitHub Secrets መሰብሰብ
        self.platforms = {
            'wordpress': {
        ...
        self.footer = "<hr/><p><em>Generated by Titan AI Enterprise v9.0</em></p>"
        self.logger.info("✅ SocialMediaManager initialized. Real-time hooks active.")

    # -------------------------------------------------------------------------
    # 🔗 ወሳኝ ድልድዮች (Methods) - ራነሩ የሚጠራቸው ስሞች
    # -------------------------------------------------------------------------

    async def publish_country_content(self, data: Dict) -> Dict:
        """ራነሩ እያንዳንዱን ሀገር (ለምሳሌ US) እንደጨረሰ የሚጠራው ዋና ዘዴ"""
        return await self._execute_realtime_publishing(data)

    async def send_production_notification(self, data: Dict, **kwargs) -> Dict:
        """በአንዳንድ የራነሩ ክፍሎች ውስጥ የሚጠራ ተጨማሪ ስም"""
        return await self._execute_realtime_publishing(data)

    async def announce_production_completion(self, data: Dict) -> Dict:
        """በምርቱ መጨረሻ ላይ (11ዱም ሲያልቁ) የሚጠራ ስም"""
        self.logger.info("📊 Production finalized. All content synchronized.")
        return {'status': 'success'}

    # -------------------------------------------------------------------------
    # 📤 ትክክለኛው የማስተላለፊያ ሎጂክ (The Actual Engine)
    # -------------------------------------------------------------------------

    async def _execute_realtime_publishing(self, data: Dict) -> Dict:
        """ሁሉንም መድረኮች የሚያስተባብር ዘዴ"""
        if not data or 'content' not in data:
            return {'status': 'error', 'message': 'No content'}

        results = {}
        country = data.get('country', 'Global')
        self.logger.info(f"📡 Real-time publishing sequence for {country} starting...")

        # 1. መላኪያ ወደ WordPress
        if self.platforms['wordpress']['enabled']:
            results['wordpress'] = await self._post_to_wordpress(data)

        # 2. መላኪያ ወደ Telegram
        if self.platforms['telegram']['enabled']:
            results['telegram'] = await self._post_to_telegram(data)

        return results

    async def _post_to_wordpress(self, data: Dict) -> Dict:
        """WordPress REST API - በቀጥታ ለህዝብ የሚያወጣ (Publish)"""
        try:
            wp = self.platforms['wordpress']
            
            # URL አስተካክል
            api_url = wp['url'].rstrip('/')
            if not api_url.endswith('/wp-json/wp/v2/posts'):
                api_url += '/wp-json/wp/v2/posts'

            # Authentication (ቤዚክ ኦውቴንቲኬሽን)
            auth_string = f"{wp['user']}:{wp['pass']}"
            token = base64.b64encode(auth_string.encode()).decode()
            
            headers = {
                'Authorization': f'Basic {token}',
                'Content-Type': 'application/json',
                'User-Agent': 'Titan-Enterprise-Runner-v9'
            }
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if r.status_code in [200, 201]:
                    ...
                    return {'status': 'success', 'url': r.json().get('link')}
                else:
                    self.logger.error(f"❌ WordPress Fail {r.status_code}: {r.text[:150]}")
                    return {'status': 'failed', 'code': r.status_code}
        except Exception as e:
            self.logger.error(f"❌ WordPress Exception: {str(e)}")
            return {'status': 'failed', 'error': str(e)}

    async def _post_to_telegram(self, data: Dict) -> Dict:
        """የቴሌግራም ማሳወቂያ መላኪያ"""
        try:
            ...
            msg = (
                ...
                f"💰 *Estimated Revenue:* ${metrics.get('estimated_revenue', 0):,.2f}/mo\n"
                f"📊 *Quality:* {metrics.get('quality_score', 0)}%"
            )
            
            url = f"https://api.telegram.org/bot{tg['token']}/sendMessage"
            async with httpx.AsyncClient(timeout=20.0) as client:
                r = await client.post(url, json={'chat_id': tg['chat_id'], 'text': msg, 'parse_mode': 'Markdown'})
                if r.status_code == 200:
                    self.logger.info(f"✅ Telegram Alert: {data.get('country')}")
                    return {'status': 'success'}
