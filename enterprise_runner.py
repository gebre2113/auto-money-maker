...
ENHANCED VERSION WITH AUTO‑FIXES FOR QUALITY, WORD COUNT, SAFETY, AND HUMAN‑LIKENESS.
ALL ORIGINAL CODE REMAINS INTACT – ONLY THE SPECIFIED IMPROVEMENTS HAVE BEEN ADDED.
"""
        ...
        try:
            ...
            if len(content.split()) > 3000:
                ...
                for i in range(0, len(content.split()), chunk_size):
                    chunk = " ".join(content.split()[i:i+chunk_size])
                    try:
...
class EnterpriseProductionOrchestrator:
    ...
    - Improved self-correction with iterative expansion to reach 15k+ words.
    - Force humanization when human score <85.
    - Regeneration based on quality feedback.
    - Auto-fixes for safety score (disclosure, alt text, links, contact).
...
"""
    ...
    async def _process_country_enterprise(self, topic: str, country: str,
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # STAGE 6: AFFILIATE LINK INJECTION (already done in bridge)
            # ------------------------------------------------------------------
            ...
            if human_score.get('human_score', 0) < 85:
                self.logger.warning(f"⚠️ Low human score ({human_score['human_score']}) - applying force humanization")
                humanized_content = await self._force_human_elements(humanized_content, country, topic)
                human_score = self.human_engine.calculate_human_score(humanized_content)
            ...
            if quality_score < self.enterprise_standards['min_quality']:
                self.logger.warning(f"⚠️ Quality below threshold ({quality_score}%) - attempting regeneration")
                improved_content = await self._regenerate_with_quality_feedback(
                    country_result['content'], quality_report, country, topic
                )
                if improved_content:
                    country_result['content'] = improved_content
                    quality_report = await asyncio.to_thread(self.quality_guardian.analyze_content, improved_content)
                    quality_score = quality_report.get('final_score', 75.0)
            ...
            if safety_check['safety_score'] < 70:
                self.logger.warning(f"⚠️ Low safety score ({safety_check['safety_score']}%) - applying auto-fixes")
                fixed_content = country_result['content']
                if not safety_check['checks']['has_affiliate_disclosure']:
                    disclosure = self.compliance_guardian._generate_affiliate_disclosure()
                    fixed_content = disclosure + "\n\n" + fixed_content
                    self.logger.info("➕ Added affiliate disclosure")
                if not safety_check['checks']['images_have_alt_text']:
                    fixed_content = self._fix_image_alt_text(fixed_content)
                    self.logger.info("🖼️ Fixed image alt text")
                if not safety_check['checks']['has_no_excessive_links']:
                    fixed_content = self._reduce_excessive_links(fixed_content)
                    self.logger.info("🔗 Reduced excessive links")
                if not safety_check['checks']['has_contact_reference']:
                    fixed_content += "\n\n" + self._generate_contact_reference()
                    self.logger.info("📞 Added contact reference")
                country_result['content'] = fixed_content
                safety_check = ProductionSafetyFeatures.validate_content_safety(fixed_content, country)
                self.logger.info(f"🔄 Safety score after fixes: {safety_check['safety_score']}%")
    ...
    # ========== ENHANCED: Improved self-correction with iterative expansion ==========
    async def _stage_5_enterprise_self_correction(self, content: str, target_words: int = 15000,
                                                  cultural_depth_score: float = 70) -> str:
        """
        🔄 Enhanced self-correction and expansion to meet word count.
        Iterates until target is reached or max attempts.
        """
        current_words = len(content.split())
        if current_words >= target_words:
            return content

        self.logger.info(f"📈 Expanding content from {current_words} to {target_words} words...")
        expanded = content
        max_iterations = 5
        iteration = 0

        while len(expanded.split()) < target_words and iteration < max_iterations:
            iteration += 1
            needed = target_words - len(expanded.split())
            chunk_target = min(2000, needed)

            prompt = f"""You are a senior business strategist. Expand the following content with a NEW, in-depth section (approximately {chunk_target} words) about advanced implementation, real-world case studies, and future trends for 2026-2027.
The new section must be completely original and not repeat ideas already present.
Use professional language, include data points, and make it actionable for enterprise readers.

Current content (last 2000 words for context):
{expanded[-4000:]}

Write the new section now:"""

            try:
                new_section = await self.failover_system.generate_content(prompt, max_tokens=4000)
                new_section = new_section.strip()
                if len(new_section.split()) < 100:
                    self.logger.warning(f"⚠️ Short expansion ({len(new_section.split())} words), retrying...")
                    continue
                expanded += f"\n\n## {self._generate_section_title()}\n\n{new_section}"
                self.logger.info(f"✅ Iteration {iteration}: added {len(new_section.split())} words")
            except Exception as e:
        ...
        if len(expanded.split()) < target_words:
            self.logger.warning(f"⚠️ Could not reach target after {max_iterations} iterations. Final: {len(expanded.split())} words")
        else:
            self.logger.info(f"✅ Target reached: {len(expanded.split())} words")

        return expanded

    def _generate_section_title(self) -> str:
        """ለአዲስ ክፍል የሚሆን ርዕስ በዘፈቀደ ይመርጣል"""
        titles = [
    ...
    # ========== ENHANCED: Force humanization when human score low ==========
    async def _force_human_elements(self, content: str, country: str, topic: str) -> str:
        """ሰው መሰልነትን በኃይል ለማሳደግ ተጨማሪ የሰው ንጥረ ነገሮችን ያስገባል"""
        prompt = f"""Rewrite the following content to sound more human, engaging, and authentic for readers in {country}.
Add personal anecdotes, expert quotes, and conversational transitions.
Do NOT change facts, just improve the style and add human touches.
...
Rewritten version:"""
        try:
            improved = await self.failover_system.generate_content(prompt, max_tokens=4000)
            if len(improved.split()) > len(content.split()) * 0.8:
                return improved
        except Exception as e:
            self.logger.warning(f"Force humanization failed: {e}")
        return content

    # ========== ENHANCED: Regenerate content based on quality feedback ==========
    async def _regenerate_with_quality_feedback(self, content: str, quality_report: Dict,
                                                country: str, topic: str) -> str:
        """የጥራት ሪፖርቱን መሠረት በማድረግ ይዘቱን እንደገና ያሻሽላል"""
        weak_layers = [k for k, v in quality_report.get('layer_scores', {}).items() if v < 80]
        if not weak_layers:
            return content

        prompt = f"""Improve the following content based on quality feedback.
Focus on improving these areas: {', '.join(weak_layers)}.
Make the content more engaging, readable, and professional.
Maintain all key information and facts.

Feedback details: {quality_report.get('recommendations', [])}
...
Improved version:"""
        ...
        try:
            improved = await self.failover_system.generate_content(prompt, max_tokens=4000)
            if len(improved.split()) > 500:
                return improved
        except Exception as e:
            self.logger.warning(f"Regeneration failed: {e}")
        return content

    # ========== HELPER: Auto-fix image alt text ==========
    def _fix_image_alt_text(self, content: str) -> str:
        """ምስሎች ላይ አልቴክስት ከሌለ በራስ-ሰር ያስገባል"""
        def add_alt(match):
            img_tag = match.group(0)
            if 'alt=' not in img_tag:
                context = content[max(0, match.start()-100):match.end()+100]
                alt_text = re.sub(r'<[^>]+>', '', context)[:50].strip()
                if not alt_text:
                    alt_text = "Image illustrating key concept"
                img_tag = img_tag.replace('<img', f'<img alt="{alt_text}"')
            return img_tag
        return re.sub(r'<img[^>]*>', add_alt, content, flags=re.IGNORECASE)

    # ========== HELPER: Reduce excessive links ==========
    def _reduce_excessive_links(self, content: str) -> str:
        """ከመጠን በላይ አገናኞችን ይቀንሳል (ከ15 በላይ ከሆነ)"""
        links = re.findall(r'<a\s+href=', content, re.IGNORECASE)
        if len(links) <= 15:
            return content
        parts = re.split(r'(<a\s+href=[^>]+>.*?</a>)', content, flags=re.IGNORECASE)
        new_parts = []
        link_count = 0
        for part in parts:
            if part.startswith('<a') and link_count >= 15:
                text = re.sub(r'<[^>]+>', '', part)
    ...
    # ========== HELPER: Generate contact reference ==========
    def _generate_contact_reference(self) -> str:
        """የዕውቂያ ማጣቀሻ ይፈጥራል"""
        return """
<div style="margin: 30px 0; padding: 20px; background: #f8fafc; border-radius: 10px; border-left: 4px solid #3b82f6;">
    <h4 style="margin-top: 0; color: #1e3a8a;">📬 ያግኙን</h4>
    <p>ለተጨማሪ መረጃ ወይም ማንኛውም ጥያቄ፣ እባክዎን በ<a href="/contact" style="color: #3b82f6;">ዕውቂያ ገጻችን</a> በኩል ያነጋግሩን። ወይም በኢሜይል <a href="mailto:support@example.com">support@example.com</a> ይጻፉልን።</p>
</div>
"""
            ...
            try:
                ...
                if country_result.get('status') == 'completed':
                    ...
                    if 'rate limit' in str(country_result.get('error', '')).lower():
                        ...
                        self._blacklist_omega_key(key_number, 86400, permanent=True)
